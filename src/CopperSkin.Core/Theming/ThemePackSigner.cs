...
public static class ThemePackSigner
{
    ...
    /// </summary>
    public static string ComputeHash(ThemePack pack) => ToHex(ComputeHashBytes(pack));
    ...
    private static string Normalize(ThemePack pack)
    {
        ...
        foreach (var pair in pack.Metadata
        ...
        foreach (var theme in pack.Themes.OrderBy(static t => t.Id, StringComparer.OrdinalIgnoreCase))
        {
            builder.Append("theme|").Append(theme.Id).Append('|').Append(theme.Name).Append('|').Append(theme.BaseThemeId).AppendLine();
            foreach (var pair in theme.Metadata.OrderBy(static p => p.Key, StringComparer.OrdinalIgnoreCase))
    ...
    public static string Sign(ThemePack pack, string signer = "CopperSkin", string? privateKeyHex = null)
    {
        ...
        var signature = BinaryFieldEcdsa.Sign(ComputeHashBytes(pack), privateKey);
    ...
    {
        if (!pack.Metadata.TryGetValue(SignatureKey, out var signature) ||
            !pack.Metadata.TryGetValue(PublicKeyKey, out var embeddedPublicKey) ||
            !string.Equals(pack.Metadata.TryGetValue(AlgorithmKey, out var algorithm) ? algorithm : string.Empty, AlgorithmValue, StringComparison.OrdinalIgnoreCase))
            return false;

        var publicKey = string.IsNullOrWhiteSpace(trustedPublicKeyHex) ? embeddedPublicKey : trustedPublicKeyHex!;
