...
7. Review [NuGet publishing](NUGET_PUBLISHING.md), then create the annotated `v0.3.0.0` tag only after the branch is merged.
8. Let GitHub Actions publish packages through NuGet Trusted Publishing with GitHub OIDC; no long-lived NuGet API key belongs in the repository.
